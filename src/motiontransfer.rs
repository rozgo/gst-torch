...
lazy_static! {
    ...
    static ref DETECTOR_MODEL: Mutex<tch::CModule> =
        Mutex::new(tch::CModule::load(env::var("SIMBOTIC_TORCH").unwrap() + "/models/motiontransfer/detector.pt").unwrap());
    static ref GENERATOR_MODEL: Mutex<tch::CModule> =
        Mutex::new(tch::CModule::load(env::var("SIMBOTIC_TORCH").unwrap() + "/models/motiontransfer/generator.pt").unwrap());
}

pub struct MotionTransfer {
    video_info: gst_video::VideoInfo,
    source_img: Tensor, // Tensor[[3, 256, 256], Uint8]
...
impl std::default::Default for MotionTransfer {
    fn default() -> Self {
        ...
        MotionTransfer {
            ...
            source_img: tch::vision::image::load(env::var("SIMBOTIC_TORCH").unwrap() + "/assets/zork_source.png")
                .unwrap()
...
impl cata::Process for MotionTransfer {
    fn process(
        ...
        let mut fake_buf = inbuf[0].copy();
        {
            ...
            let fake_ref = fake_buf.get_mut().unwrap();
            let mut out_frame =
                gst_video::VideoFrameRef::from_buffer_ref_writable(fake_ref, &self.video_info)
                    .unwrap();
            ...
            if self.kp_source == None {
                let source = self.source_img.to_kind(tch::Kind::Float) / 255;
                let source = source.unsqueeze(0);
                self.source = Some(source.copy());
                let source = tch::IValue::Tensor(source);
                let detector_output = DETECTOR_MODEL.lock().unwrap().forward_is(&[source]).unwrap();
            ...
            if self.kp_driving_initial == None {
                ...
                let detector_output = DETECTOR_MODEL.lock().unwrap().forward_is(&[driving_initial]).unwrap();
                let det_tensors = match &detector_output {
                    tch::IValue::Tuple(det_tensors) => Some(det_tensors),
                    _ => None,
                }
            ...
            let mut kp_driving: Option<(Tensor, Tensor)> = None;
            {
                ...
                let detector_output = DETECTOR_MODEL.lock().unwrap().forward_is(&[kp_driving_img]).unwrap();
                let det_tensors = match &detector_output {
                    tch::IValue::Tuple(det_tensors) => Some(det_tensors),
                    _ => None,
                }
            ...
            match (&self.source, &self.kp_source, &kp_driving, &self.kp_driving_initial) {
                (Some(source),
                    ...
                    let jacobian_diff = kp_driving_jacobian.matmul(&kp_driving_initial_jacobian_inv);
                    ...
                    let gen_pred = GENERATOR_MODEL.lock().unwrap().forward_is(&[
                        source,
            ...
            if let Some(prediction) = prediction {
                ...
                .permute(&[1, 2, 0])
                .copy_data(fake_out, (WIDTH * HEIGHT * 3) as usize);
            }
        }
